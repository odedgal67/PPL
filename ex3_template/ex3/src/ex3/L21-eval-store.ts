...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] = map((v:Value) : number =>{
        return extendStore(theStore,v).vals.length-1;
    },args)
...
{
    const varValue : CExp = def.val;
    const varValueResult : Result<Value> = applicativeEval(varValue, theGlobalEnv);
    if(isOk(varValueResult))
    {
        const varName : string = def.var.var;
        const extndedStore: Store = extendStore(theStore, varValueResult.value);
        const newVarAddress: number = extndedStore.vals.length-1; //Address of last variable on the array 
        globalEnvAddBinding(varName, newVarAddress);
        return evalSequence(exps, theGlobalEnv);
    }
    else
    {
        return makeFailure("Value evaluation failed");
    }
...
{
    const vals = mapResult((v: CExp) => applicativeEval(v, env), map((b: Binding) => b.val, exp.bindings)); //Array of values
    const vars = map((b: Binding) => b.var.var, exp.bindings);  //Array of variable's names
