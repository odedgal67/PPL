...
describe('Q3 Tests', () => {
     it('test parse/unparse class', () => {
          ...
          expect(bind(bind(p(`(class (a b) ((first (lambda () a)) (second (lambda () b)) (sum (lambda () (+ a b))) (son (lambda () (class (c d) ((third (lambda () c)) (fourth (lambda () c)) (end (lambda () c))))))))`),parseL31Exp), x=>makeOk(unparseL31(x)))).to.deep.equal(makeOk(`(class (a b) ((first (lambda () a)) (second (lambda () b)) (sum (lambda () (+ a b))) (son (lambda () (class (c d) ((third (lambda () c)) (fourth (lambda () c)) (end (lambda () c))))))))`));
         });
     ...
     it('trnasform class-exp program in to proc-exp', () => {
          ...
          expect(bind(bind(parseL31(`(L31 (define pair (class (a) ((first (lambda () a)) (second (lambda () (class (b) ((f (lambda () b))))))))))`), L31ToL3),  x=>makeOk(unparseL31(x)))).to.deep.equal(makeOk(`(L31 (define pair (lambda (a) (lambda (msg) (if (eq? msg 'first) ((lambda () a) ) (if (eq? msg 'second) ((lambda () (lambda (b) (lambda (msg) (if (eq? msg 'f) ((lambda () b) ) #f)))) ) #f))))))`));
     });
